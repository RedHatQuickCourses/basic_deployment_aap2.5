...
The above is an example of cluster setup and there is no need to explicitly tell the script as it will take care of it automatically. This behavior is the same for different components apart from the database. 
...
.. Higher computing capacity for large volumes of automation.
.. Redundancy and high availability across platform components.
.. Scalable automation mesh for managing distributed environments.
+
...
[IMPORTANT]
====
For production, Red Hat recommends enterprise topology with an external database for performance and availability.
====
...
image::4.2.png[]
+
The above will be deployed on a single system itself where 8443 will be the port for the *Unified UI* to be accessed.
...
- Good for: Development, testing, small production (<50 managed nodes)
