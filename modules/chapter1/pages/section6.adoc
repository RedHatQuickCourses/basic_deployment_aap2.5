...
. *Operator growth Topology*: The growth topology is intended for organizations that are getting started with Ansible Automation Platform and do not require redundancy or higher compute for large volumes of automation. This topology allows for smaller footprint deployments.
+
image::6.1.png[]
...
The benefits of using this topology is: 

- Limited redundancy.
- Lower upfront resource requirements (fewer nodes, shared components).


. *Operator enterprise Topology* : The enterprise topology is intended for organizations that require Ansible Automation Platform to be deployed with redundancy or higher compute for large volumes of automation. 
+
image::6.2.png[]
