...
Both standalone and cluster systems are quite easy to setup. If there is only one hostname listed under the component name, it is a standalone configuration; if there are several hostnames listed under the single group name, it is a cluster setup. All you need to do is add the systems under the group name of the component you wish to install.
...
The above will be a standalone set up. Once the installation script will run it will automatically take care of that. 
...
The above is a cluster setup and there is no need to explicitly tell it to the script as it will take care of it automatically. And this behavior is the same for different components apart from the database. 
